                    ...
                    <div class="table-responsive table mt-2" id="dataTable-2" role="grid"
                        ...
                        <table class="table my-0" id="dataTable">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td style="width: 264.828px;">{{$invite->user->firstname}}
                                        {{$invite->user->lastname}}</td>
                                    <td>{{$invite->user->birthdate}}</td>
                                    <td>{{$invite->user->course}}</td>
                                    <td></td>
                                    <td>{{$invite->invite_message}}</td>
                                    <td>{{ ($invite->status == null ? 'PENDING' : ($invite->status == 1 ? 'ACCEPTED' :
                                        'REJECTED')) }}</td>
                                </tr>
                                @endforeach
                            </tbody>
                    ...
                    <!--<div class="row">
                        <div class="col-md-6 align-self-center">
                            ...
                            <p id="dataTable_info-2" class="dataTables_info" role="status" aria-live="polite"
                                style="margin-bottom: 0px;color: rgb(78,115,223);">Game Category: Sports</p>
                ...
                <form class="user" method="POST" action="{{url('add-member-team')}}">
                    ...
                    <div class="row mb-3" style="margin-top: -50px;">
                        <div class="col-sm-6 mb-3 mb-sm-0" style="margin-top: 35px;">
                            <input class="form-control form-control-user" type="hidden" name="team_id"
                                value="{{ request()->id }}"
                                style="width: 450px;height: 50px;border-radius: 10px;margin-bottom: 15px;">
                            {{-- <input class="form-control form-control-user" type="hidden" name="invite_status"
                                value="SENT INVITE"
                                style="width: 450px;height: 50px;border-radius: 10px;margin-bottom: 15px;"> --}}
                            <label>Select player/user<label>
                                    <select class="form-select" name="user_id"
                                        style="height: 50px;padding-top: 3px;padding-bottom: 3px;font-size: 12px;width: 450px;border-radius: 10px;">
                                        <option value="0" selected="">Select a User to add to the team</option>
                                        @foreach($users as $user)
                                        <option value="{{$user->id}}">{{$user->course}}: {{$user->firstname}}
                                            {{$user->lastname}}</option>
                                        @endforeach
                                    </select>
                                    <label>Leave a message<label>
                                            <input class="form-control form-control-user" type="text"
                                                name="invite_message" placeholder="Invite message" value=""
                                                style="width: 450px;height: 50px;border-radius: 10px;margin-bottom: 15px;">

                        </div>
                    </div>
                    <div class="mb-3"></div>
                    <div class="text-center"></div><button class="btn btn-primary d-block btn-user" type="submit"
                        style="margin-top: 25px;width: 150px;margin-left: 300px;"><i class="fas fa-users-cog"
                            style="margin-right: 10px;"></i>Send Invitation</button>
